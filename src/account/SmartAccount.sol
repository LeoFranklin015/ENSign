 ...
 */
contract SmartAccount is BaseAccount, MultiOwnable, IERC165, Receiver, ERC1271 {
     ...
     */
    error SmartAccount_AlreadyInitialized();

    /**
     * @notice Thrown when an unauthorized caller attempts to initialize the account.
     */
    error SmartAccount_UnauthorizedInitialization();
     ...
     */
    error SmartAccount_SelectorNotAllowed(bytes4 selector);
     ...
     */
    error SmartAccount_InvalidNonceKey(uint256 key);
    ...
    /**
     * @notice Initializes the SmartAccount contract with the entry point and factory addresses.
    ...
    function executeWithoutChainIdValidation(bytes[] calldata calls) external payable virtual {
        ...
        for (uint256 i; i < callsLength; i++) {
            ...
            if (!canSkipChainIdValidation(selector)) {
                revert SmartAccount_SelectorNotAllowed(selector);
            }
        ...
        returns (uint256 validationData)
    {
        ...
        if (bytes4(userOp.callData) == this.executeWithoutChainIdValidation.selector) {
            ...
            if (key != REPLAYABLE_NONCE_KEY) {
                revert SmartAccount_InvalidNonceKey(key);
            }
        } else {
            if (key == REPLAYABLE_NONCE_KEY) {
                revert SmartAccount_InvalidNonceKey(key);
    ...
    /**
     * @notice Initializes the SmartAccount with the provided owners.
     ...
     * @return name The domain name "SmartAccount".
